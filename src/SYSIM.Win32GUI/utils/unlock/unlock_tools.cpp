...
namespace UnlockTools {
    ...
    // ---- Вспомогательные функции (только внутренние, не объявлены в .h) ----
    namespace {
        ...
        bool IsLikelyWinRE() {
            wchar_t sd[16] = {};
            if (GetEnvironmentVariableW(L"SystemDrive", sd, 16) > 0 && _wcsicmp(sd, L"X:") == 0) return true;

            HKEY h = nullptr;
            if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\MiniNT", 0, KEY_READ, &h) == ERROR_SUCCESS) {
                RegCloseKey(h);
                return true;
            }
            return false;
        ...
        // ==== Файловые хелперы (должны быть ДО MountOfflineWindows/UnmountOfflineWindows) ====
        ...
        // ==== Офлайн-монтирование (должно быть после IsDirectory/IsFile) ====
        static void UnmountOfflineWindows(bool showErrors);
        static bool MountOfflineWindows(const std::wstring& windowsPathRaw);

        // ==== DisallowRun / IFEO через WinCtrl ====
        bool CleanDisallowRunAt(HKEY root, const std::wstring& explorerSubKey) {
            if (root == HKEY_CURRENT_USER) {
                WinCtrl::Registry::DeleteKey(HKEY_CURRENT_USER, explorerSubKey + L"\\DisallowRun");
                WinCtrl::Registry::DeleteValue(HKEY_CURRENT_USER, explorerSubKey, L"DisallowRun");
                return true;
            }
            else if (root == HKEY_LOCAL_MACHINE) {
                WinCtrl::Registry::DeleteKey(HKEY_LOCAL_MACHINE, explorerSubKey + L"\\DisallowRun");
                WinCtrl::Registry::DeleteValue(HKEY_LOCAL_MACHINE, explorerSubKey, L"DisallowRun");
                return true;
            }
            return false;
        }
        ...
        bool CleanIFEOAt(HKEY root, const std::wstring& ifeoSubKey) {
            if (root == HKEY_LOCAL_MACHINE) {
                WinCtrl::Registry::DeleteKey(HKEY_LOCAL_MACHINE, ifeoSubKey);
                return true;
            }
            return false;
        }
        ...
        bool HasDisallowRunAt(HKEY root, const std::wstring& explorerSubKey) {
            if (root == HKEY_CURRENT_USER) {
                if (WinCtrl::Registry::KeyExists(HKEY_CURRENT_USER, explorerSubKey + L"\\DisallowRun"))
                    return true;
                DWORD val = 0;
                if (WinCtrl::Registry::ReadDWORD(HKEY_CURRENT_USER, explorerSubKey, L"DisallowRun", val) && val != 0)
                    return true;
            }
            else if (root == HKEY_LOCAL_MACHINE) {
                if (WinCtrl::Registry::KeyExists(HKEY_LOCAL_MACHINE, explorerSubKey + L"\\DisallowRun"))
                    return true;
                DWORD val = 0;
                if (WinCtrl::Registry::ReadDWORD(HKEY_LOCAL_MACHINE, explorerSubKey, L"DisallowRun", val) && val != 0)
                    return true;
            }
            return false;
        }
        ...
        bool HasIFEODebuggerAt(HKEY root, const std::wstring& ifeoSubKey) {
            if (root == HKEY_LOCAL_MACHINE) {
                auto subKeys = WinCtrl::Registry::EnumerateSubKeys(HKEY_LOCAL_MACHINE, ifeoSubKey);
                for (const auto& sub : subKeys) {
                    std::wstring full = ifeoSubKey + L"\\" + sub;
                    if (WinCtrl::Registry::ValueExists(HKEY_LOCAL_MACHINE, full, L"Debugger"))
                        return true;
                }
            }
            return false;
        ...
        bool IsWinlogonShellHijacked() {
            std::wstring shell;
            if (!WinCtrl::Registry::ReadString(HKEY_LOCAL_MACHINE, kWinlogonKey, L"Shell", shell))
                return false;
            std::wstring lower = shell;
            for (auto& c : lower) if (c >= L'A' && c <= L'Z') c = c - L'A' + L'a';
            return lower.find(L"explorer.exe") == std::wstring::npos;
        }

        bool FixWinlogonShell() {
            bool ok = true;
            ok &= WinCtrl::Persistence::ResetWinlogonShell();
            ok &= WinCtrl::Persistence::ResetWinlogonUserinit();
            ok &= WinCtrl::Registry::DeleteValue(HKEY_LOCAL_MACHINE, kWinlogonKey, L"Taskman");
            return ok;
        }

        bool HasLegalNotice() {
            std::wstring s;
            if (WinCtrl::Registry::ReadString(HKEY_LOCAL_MACHINE, kWinlogonKey, L"LegalNoticeCaption", s) && !s.empty())
                return true;
            if (WinCtrl::Registry::ReadString(HKEY_LOCAL_MACHINE, kWinlogonKey, L"LegalNoticeText", s) && !s.empty())
                return true;
            return false;
        }
        ...
        bool ClearLegalNotice() {
            bool ok = true;
            ok &= WinCtrl::Registry::DeleteValue(HKEY_LOCAL_MACHINE, kWinlogonKey, L"LegalNoticeCaption");
            ok &= WinCtrl::Registry::DeleteValue(HKEY_LOCAL_MACHINE, kWinlogonKey, L"LegalNoticeText");
            return ok;
        }
        ...
        bool AreExeAssociationsHijacked() {
            std::wstring cmd;
            if (!WinCtrl::Registry::ReadString(HKEY_CLASSES_ROOT, L"exefile\\shell\\open\\command", L"", cmd))
                return true;
        ...
        bool FixExeAssociations() {
            ...
            for (const A& a : items) {
                ...
                if (!WinCtrl::Registry::WriteString(HKEY_CLASSES_ROOT, sub, L"", a.cmd))
                    ok = false;
            }
            return ok;
        }
        ...
        // ==== Proxy ====
        static const wchar_t* kInetKey = L"Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings";

        bool IsProxyForced() {
            DWORD v = 0;
            if (WinCtrl::Registry::ReadDWORD(HKEY_CURRENT_USER, kInetKey, L"ProxyEnable", v) && v != 0)
                return true;
            std::wstring s;
            if (WinCtrl::Registry::ReadString(HKEY_CURRENT_USER, kInetKey, L"AutoConfigURL", s) && !s.empty())
                return true;
            return false;
        }
        ...
        bool ResetProxy() {
            bool ok = true;
            ok &= WinCtrl::Registry::WriteDWORD(HKEY_CURRENT_USER, kInetKey, L"ProxyEnable", 0);
            ok &= WinCtrl::Registry::DeleteValue(HKEY_CURRENT_USER, kInetKey, L"ProxyServer");
            ok &= WinCtrl::Registry::DeleteValue(HKEY_CURRENT_USER, kInetKey, L"AutoConfigURL");
            return ok;
        }
        ...
        bool AreHiddenFilesForced() {
            DWORD v = 0;
            return WinCtrl::Registry::ReadDWORD(HKEY_CURRENT_USER, kAdvKey, L"Hidden", v) && v == 2;
        }

        bool RestoreHiddenFiles() {
            bool ok = true;
            ok &= WinCtrl::Registry::WriteDWORD(HKEY_CURRENT_USER, kAdvKey, L"Hidden", 1);
            ok &= WinCtrl::Registry::WriteDWORD(HKEY_CURRENT_USER, kAdvKey, L"ShowSuperHidden", 1);
            ok &= WinCtrl::Registry::WriteDWORD(HKEY_CURRENT_USER, kAdvKey, L"HideFileExt", 0);
            return ok;
        }

        // ==== AppInit_DLLs ====
        static const wchar_t* kWinKey = L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Windows";

        bool HasAppInitDlls() {
            std::wstring s;
            if (WinCtrl::Registry::ReadString(HKEY_LOCAL_MACHINE, kWinKey, L"AppInit_DLLs", s) && !s.empty())
                return true;
            DWORD v = 0;
            if (WinCtrl::Registry::ReadDWORD(HKEY_LOCAL_MACHINE, kWinKey, L"LoadAppInit_DLLs", v) && v != 0)
                return true;
            return false;
        }
        ...
        bool ClearAppInitDlls() {
            bool ok = true;
            ok &= WinCtrl::Persistence::SetAppInitDLLs({});
            ok &= WinCtrl::Registry::WriteDWORD(HKEY_LOCAL_MACHINE, kWinKey, L"LoadAppInit_DLLs", 0);
            return ok;
        }
        ...
        bool AreDefenderServicesDisabled() {
            DWORD v = 0;
            return WinCtrl::Registry::ReadDWORD(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\WinDefend", L"Start", v) && v == 4;
        }
        ...
        bool RestoreDefenderServices() {
            ...
            for (const S& s : svc) {
                std::wstring sub = L"SYSTEM\\CurrentControlSet\\Services\\" + std::wstring(s.name);
                if (!WinCtrl::Registry::WriteDWORD(HKEY_LOCAL_MACHINE, sub, L"Start", s.start)) ok = false;
            }
            if (!WinCtrl::Security::EnableDefender()) ok = false;
            return ok;
        }

        // ==== hosts ====
        bool IsHostsHijackedAt(const std::wstring& hostsPath) {
            HANDLE h = CreateFileW(hostsPath.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
            ...
            while (pos < text.size()) {
                ...
                std::wstring line = text.substr(pos, (nl == std::wstring::npos) ? std::wstring::npos : nl - pos);
        ...
        bool WriteDefaultHosts(const std::wstring& hostsPath) {
            ...
            HANDLE h = CreateFileW(hostsPath.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
            if (h == INVALID_HANDLE_VALUE) return false;
        ...
        std::wstring BcdObject() {
            return IsLikelyWinRE() ? L"{default}" : L"{current}";
        }
        ...
        bool ApplyOfflineFix(HKEY root, const OfflineFix& fix) {
            ...
            if (fix.deleteValue)
                result = RegDeleteValueW(hKey, fix.valueName.c_str());
            else
                result = RegSetValueExW(hKey, fix.valueName.c_str(), 0, REG_DWORD, reinterpret_cast<const BYTE*>(&fix.value), sizeof(fix.value));
        ...
        void ForEachUserHive(const std::wstring& usersDir, Fn callback) {
            ...
            do {
                ...
                if (userName == L"." || userName == L".." || userName == L"Public" ||
                    userName == L"All Users" || userName == L"Default" || userName == L"Default User") continue;
        ...
        bool ReadDwordOffline(HKEY root, const std::wstring& subKey, const std::wstring& valueName, DWORD& outValue) {
            if (!root) return false;
            HKEY hKey = nullptr;
            if (RegOpenKeyExW(root, subKey.c_str(), 0, KEY_READ, &hKey) != ERROR_SUCCESS) return false;

            DWORD type = 0, size = sizeof(outValue);
            LONG st = RegQueryValueExW(hKey, valueName.c_str(), nullptr, &type, reinterpret_cast<LPBYTE>(&outValue), &size);
            RegCloseKey(hKey);
            return st == ERROR_SUCCESS && type == REG_DWORD;
        }

        void CheckOfflineValue(HKEY root, const std::wstring& subKey, const std::wstring& valueName,
            const std::wstring& description, bool zeroMeansBlocked,
            std::wstring& report, int& blockedCount) {
            DWORD value = 0;
        ...
        int ScanOfflineWindows(std::wstring& report) {
            ...
            for (const auto& item : softwareItems)
                CheckOfflineValue(hSoft, item.subKey, item.valueName, item.description, item.zeroMeansBlocked, report, blockedCount);
            ...
            if (hFind != INVALID_HANDLE_VALUE) {
                ...
                do {
                    ...
                    for (const auto& item : userItems)
                        CheckOfflineValue(hUser, item.subKey, item.valueName, item.description, item.zeroMeansBlocked, report, blockedCount);
        ...
        // ==== Offline repair ====
        bool RepairOfflineWindowsInternal() {
            EnableOfflinePrivileges();
            std::wstring winDrive = FindOfflineWindowsDrive();
            ...
            for (const auto& fix : softwareFixes)
                ApplyOfflineFix(hSoft, fix);

            {
                HKEY hWl = nullptr;
                if (RegOpenKeyExW(hSoft, L"Microsoft\\Windows NT\\CurrentVersion\\Winlogon", 0, KEY_SET_VALUE, &hWl) == ERROR_SUCCESS) {
                    const wchar_t* shell = L"explorer.exe";
                    RegSetValueExW(hWl, L"Shell", 0, REG_SZ, reinterpret_cast<const BYTE*>(shell), (DWORD)((wcslen(shell) + 1) * sizeof(wchar_t)));

                    const wchar_t* userinit = L"C:\\Windows\\system32\\userinit.exe,";
                    RegSetValueExW(hWl, L"Userinit", 0, REG_SZ, reinterpret_cast<const BYTE*>(userinit), (DWORD)((wcslen(userinit) + 1) * sizeof(wchar_t)));
            ...
            ForEachUserHive(usersDir, [&](HKEY hUser) {
                for (const auto& fix : userFixes)
                    ApplyOfflineFix(hUser, fix);
            ...
            if (RegLoadAppKeyW(systemPath.c_str(), &hSys, KEY_ALL_ACCESS, 0, 0) == ERROR_SUCCESS) {
                ...
                for (const auto& s : svcFixes) {
                    ApplyOfflineFix(hSys, { std::wstring(L"ControlSet001\\Services\\") + s.name, L"Start", s.start, false });
                }
                RegFlushKey(hSys);
                RegCloseKey(hSys);
            }
            ...
            if (IsHostsHijackedAt(hostsPath))
                WriteDefaultHosts(hostsPath);

            // Удаляем SRP/AppLocker
            {
                HKEY hKey = nullptr;
                if (RegOpenKeyExW(hSoft, L"Policies\\Microsoft\\Windows\\Safer", 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS) {
                    RegDeleteTreeW(hKey, nullptr);
                    RegCloseKey(hKey);
                    RegDeleteKeyW(hSoft, L"Policies\\Microsoft\\Windows\\Safer");
                }
                if (RegOpenKeyExW(hSoft, L"Policies\\Microsoft\\Windows\\SrpV2", 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS) {
                    RegDeleteTreeW(hKey, nullptr);
                    RegCloseKey(hKey);
                    RegDeleteKeyW(hSoft, L"Policies\\Microsoft\\Windows\\SrpV2");
                }
            }
        ...
        bool RunCapture(const std::wstring& command, std::wstring& log) {
            ...
            if (!ok) {
                CloseHandle(hRead);
                log += L"[ошибка запуска] " + command + L"\r\n";
                return false;
            }
    ...
    // ==================== ПУБЛИЧНЫЕ ФУНКЦИИ (объявлены в .h) ====================
    ...
    std::vector<Restriction> GetKnownRestrictions() {
        ...
        add(L"Диспетчер задач", both, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableTaskMgr", 0, false);
        add(L"Редактор реестра", both, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableRegistryTools", 0, false);
        add(L"Командная строка (CMD)", both, L"Software\\Policies\\Microsoft\\Windows\\System", L"DisableCMD", 0, false);
        add(L"Командная строка (CMD, старый путь)", both, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableCMD", 0, false);
        add(L"UAC (Контроль учётных записей)", hklm, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"EnableLUA", 1, false);
        add(L"Панель управления", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoControlPanel", 0, false);
        add(L"Окно 'Выполнить' (Win+R)", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoRun", 0, false);
        add(L"Доступ к дискам", both, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoViewOnDrive", 0, false);
        add(L"Скрытие дисков", both, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoDrives", 0, false);
        add(L"Поиск в Пуске", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoFind", 0, false);
        add(L"Контекстное меню", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoViewContextMenu", 0, false);
        add(L"Параметры папок", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoFolderOptions", 0, false);
        add(L"Вкладка 'Безопасность'", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoSecurityTab", 0, false);
        add(L"Меню 'Файл'", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoFileMenu", 0, false);
        add(L"Кнопка выключения", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoClose", 0, false);
        add(L"Выход из системы", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoLogoff", 0, false);
        add(L"Смена обоев", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\ActiveDesktop", L"NoChangingWallPaper", 0, false);
        add(L"Персонализация (NoDispCPL)", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"NoDispCPL", 0, false);
        add(L"Настройки фона (NoDispBackgroundPage)", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"NoDispBackgroundPage", 0, false);
        add(L"Горячие клавиши Win", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoWinKeys", 0, false);
        add(L"Настройки панели задач", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoSetTaskbar", 0, false);
        add(L"Иконки рабочего стола (NoDesktop)", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", L"NoDesktop", 0, false);
        add(L"Блокировка рабочей станции", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableLockWorkstation", 0, false);
        add(L"Смена пароля", hkcu, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableChangePassword", 0, false);
        add(L"Восстановление системы", hklm, L"Software\\Policies\\Microsoft\\Windows NT\\SystemRestore", L"DisableSR", 0, false);
        add(L"Установщик MSI (DisableMSI)", hklm, L"Software\\Policies\\Microsoft\\Windows\\Installer", L"DisableMSI", 0, false);
        add(L"PowerShell (скрипты запрещены)", both, L"Software\\Policies\\Microsoft\\Windows\\PowerShell", L"DisableRunScripts", 0, true);
        add(L"Defender (DisableAntiSpyware)", hklm, L"Software\\Policies\\Microsoft\\Windows Defender", L"DisableAntiSpyware", 0, false);
        add(L"Defender (DisableAntiVirus)", hklm, L"Software\\Policies\\Microsoft\\Windows Defender", L"DisableAntiVirus", 0, false);
        add(L"Defender (мониторинг в реальном времени)", hklm, L"Software\\Policies\\Microsoft\\Windows Defender\\Real-Time Protection", L"DisableRealtimeMonitoring", 0, false);
        add(L"MMC (только разрешённые оснастки)", both, L"Software\\Policies\\Microsoft\\MMC", L"RestrictToPermittedSnapins", 0, false);
    ...
    bool IsRestricted(const Restriction& r) {
        for (HKEY hive : r.hives) {
            DWORD value = 0;
            if (WinCtrl::Registry::ReadDWORD(hive, r.subKey, r.valueName, value)) {
    ...
    bool UnlockRestriction(const Restriction& r) {
        bool ok = true;
        for (HKEY hive : r.hives) {
            if (r.deleteInsteadOfSet) {
                if (!WinCtrl::Registry::DeleteValue(hive, r.subKey, r.valueName)) ok = false;
            }
            else {
                if (!WinCtrl::Registry::WriteDWORD(hive, r.subKey, r.valueName, r.disableValue)) ok = false;
            }
        }
        return ok;
    }
    ...
    bool ClearImageFileExecutionOptions() {
        return CleanIFEOAt(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options");
    }

    bool RepairOfflineWindows() {
        return RepairOfflineWindowsInternal();
    }
    ...
    bool ClearOfflineIFEO() {
        ...
        HKEY hSoft = nullptr;
        if (RegLoadAppKeyW((winDrive + L"Windows\\System32\\config\\SOFTWARE").c_str(), &hSoft, KEY_ALL_ACCESS, 0, 0) != ERROR_SUCCESS)
            return false;
    ...
    PolicyLocks ScanPolicyLocks() {
        PolicyLocks r;
        if (WinCtrl::Registry::KeyExists(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\Safer\\CodeIdentifiers")) {
            DWORD level = 0;
            if (WinCtrl::Registry::ReadDWORD(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\Safer\\CodeIdentifiers", L"DefaultLevel", level) && level != 262144) {
                r.srp = true;
            }
            if (WinCtrl::Registry::KeyExists(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\Safer\\CodeIdentifiers\\0") ||
                WinCtrl::Registry::KeyExists(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\Safer\\CodeIdentifiers\\4096")) {
                r.srp = true;
            }
        }
        if (WinCtrl::Registry::KeyExists(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\SrpV2")) {
            r.applocker = true;
            for (const wchar_t* cat : { L"Exe", L"Dll", L"Script", L"Msi", L"Appx" }) {
                std::wstring sub = std::wstring(L"SOFTWARE\\Policies\\Microsoft\\Windows\\SrpV2\\") + cat;
                DWORD mode = 0;
                if (WinCtrl::Registry::ReadDWORD(HKEY_LOCAL_MACHINE, sub, L"EnforcementMode", mode) && mode == 1) {
    ...
    void UnlockPolicyLocks() {
        WinCtrl::Registry::DeleteKey(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\Safer");
        WinCtrl::Registry::DeleteKey(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\Microsoft\\Windows\\SrpV2");
    }
    ...
    BootInfo ScanBoot() {
        ...
        if (!b.windowsDir.empty()) {
            b.winloadOk = FileExistsPath(b.windowsDir + L"\\System32\\winload.exe") || FileExistsPath(b.windowsDir + L"\\System32\\winload.efi");
        }
    ...
    bool IsRegistryEditorLocked() {
        DWORD value = 0;
        return WinCtrl::Registry::ReadDWORD(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableRegistryTools", value) && value != 0;
    }

    bool UnlockRegistryEditor() {
        return WinCtrl::Registry::WriteDWORD(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableRegistryTools", 0);
    }

    bool IsTaskManagerLocked() {
        DWORD value = 0;
        return WinCtrl::Registry::ReadDWORD(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableTaskMgr", value) && value != 0;
    }

    bool UnlockTaskManager() {
        return WinCtrl::Registry::WriteDWORD(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", L"DisableTaskMgr", 0);
    }

    bool IsUACDisabled() {
        return !WinCtrl::UAC::IsEnabled();
    }

    bool EnableUAC() {
        bool ok = WinCtrl::UAC::Enable();
        WinCtrl::System::ForceGPUpdate();
        return ok;
    }
    ...
    std::wstring GetBestUnlockReport(bool unlock) {
        ...
        if (offlineBlocked >= 0) {
            ...
            if (offlineBlocked == 0)
                body += L"Offline: активные блокировки не найдены.\r\n";

            if (unlock) {
                if (RepairOfflineWindows()) {
                    if (offlineBlocked > 0) {
                        body += L"Offline: все найденные блокировки сняты.\r\n";
                        unlockedCount += offlineBlocked;
                    }
                    else {
                        body += L"Offline: выполнена профилактическая разблокировка.\r\n";
                        ++unlockedCount;
                    }
                }
                else {
                    body += L"Offline: не удалось выполнить разблокировку.\r\n";
                    ++failedCount;
                }
            }
            completed = true;
        ...
        if (!completed) {
            ...
            for (const auto& r : restrictions) {
                ...
                if (!unlock) {
                    body += L"Заблокировано: " + r.description + L"\r\n";
                    continue;
                }

                if (UnlockRestriction(r)) {
                    body += L"Разблокировано: " + r.description + L"\r\n";
                    ++unlockedCount;
                }
                else {
                    body += L"Ошибка: " + r.description + L"\r\n";
                    ++failedCount;
                }
            }
            ...
            if (pl.srp) {
                ++blockedCount;
                if (!unlock)
                    body += L"Заблокировано: Software Restriction Policies (SRP)\r\n";
            }
            if (pl.applocker) {
                ++blockedCount;
                if (!unlock)
                    body += L"Заблокировано: AppLocker" + std::wstring(pl.applockerEnforced ? L" (принудительно)" : L"") + L"\r\n";
            }

            if (unlock && (pl.srp || pl.applocker)) {
                UnlockPolicyLocks();
                if (pl.srp) {
                    body += L"Разблокировано: SRP\r\n";
                    ++unlockedCount;
                }
                if (pl.applocker) {
                    body += L"Разблокировано: AppLocker\r\n";
                    ++unlockedCount;
                }
            }

            bool hasDisallowRun = HasDisallowRunAt(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer") ||
                HasDisallowRunAt(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer");
            bool hasIFEO = HasIFEODebuggerAt(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options");

            if (hasDisallowRun) {
                ++blockedCount;
                if (!unlock)
                    body += L"Заблокировано: DisallowRun\r\n";
            }
            if (hasIFEO) {
                ++blockedCount;
                if (!unlock)
                    body += L"Заблокировано: IFEO Debugger\r\n";
            }

            if (unlock) {
                if (hasDisallowRun) {
                    if (ClearDisallowRun()) {
                        body += L"Разблокировано: DisallowRun\r\n";
                        ++unlockedCount;
                    }
                    else {
                        body += L"Ошибка: DisallowRun\r\n";
                        ++failedCount;
                    }
                }
                if (hasIFEO) {
                    if (ClearImageFileExecutionOptions()) {
                        body += L"Разблокировано: IFEO Debugger\r\n";
                        ++unlockedCount;
                    }
                    else {
                        body += L"Ошибка: IFEO Debugger\r\n";
                        ++failedCount;
                    }
                }
            }
            ...
            static const ActionItem kActions[] = {
                ...
                { L"Скрытые файлы не показываются (Исправление включит показ)", AreHiddenFilesForced, RestoreHiddenFiles, false },
            ...
            for (const auto& a : kActions) {
                ...
                if (!unlock) {
                    body += std::wstring(L"") + (a.counts ? L"Заблокировано: " : L"Инфо: ") + a.desc + L"\r\n";
                    continue;
                }
                ...
                if (a.fix()) {
                    ...
                    body += L"Ошибка: " + std::wstring(a.desc) + L"\r\n";
            ...
            if (blockedCount == 0)
                body += L"Активные блокировки не обнаружены.\r\n";
        }
